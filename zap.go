...
func main() {
  ...
  if err != nil { log.Fatalf("server.Bind failed: %s\n", err) }
  ...
  if err != nil { log.Fatalf("client.SendFrame failed: %s\n", err) }
  ...
  if err != nil { log.Fatalf("server.RecvFrame failed: %s\n", err) }

  err = server.SendFrame(frame, 0)
  if err != nil { log.Fatalf("server.SendFrame failed: %s\n", err) }

  frame, _, err = client.RecvFrame()
  if err != nil { log.Fatalf("client.RecvFrame failed: %s\n", err) }
